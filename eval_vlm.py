...
def init_model(args):
    ...
    if 'model' in args.load_from:
        ...
        model.vision_encoder, model.processor = Qwen3VLM.get_vision_model("./model/vision_model/clip-vit-base-patch16")
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3-V Chat")
    ...
    for image_file in sorted(os.listdir(args.image_dir)):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            pixel_values = Qwen3VLM.image2tensor(image, preprocess).to(args.device).unsqueeze(0)
